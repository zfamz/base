<template>
  <div class="basic-item" :class="{'close': !show}">
    ...
    <el-form ref="licenseUse" :model="form[index]" label-width="80px" v-for="(item,index) in count" :key="item" class="form-item" :disabled="type === 'view'" v-show="show">
      <el-row>
        ...
        <base-form-item label="取得方式" prop="obtainType">
          <el-input v-model="form[index].obtainType" placeholder="请输入"></el-input>
        </base-form-item>
        ...
        <base-form-item label="用地面积" prop="useArea" rule="_number">
          <el-input v-model="form[index].useArea" placeholder="请输入" rule="_number">
        ...
        <base-form-item label="他项权利" prop="otherRights">
          <el-input v-model="form[index].otherRights" placeholder="请输入"></el-input>
        </base-form-item>
      </el-row>
  ...
  methods: {
    ...
    getForm() {
      ...
      for (let i = 0; i < this.count; i++) {
        ...
        this.$refs.licenseUse[i].validate((valid) => {
          if (!valid) {
    ...
    setForm(data) {
      ...
      // for (let i = 0; i < this.count; i++) {
      //   this.$refs.licenseUse[i].resetFields();
      // }
    }
  }
}
